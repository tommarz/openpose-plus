...
def model(x, n_pos, mask_miss1, mask_miss2, is_train=False, train_bn = False, reuse=None, data_format='channels_last'):  # hao25

    def depthwise_conv_block(n, n_filter, filter_size=(3, 3), strides=(1, 1), name="depth_block"):
        with tf.variable_scope(name):
            n = DepthwiseConv2d(n, filter_size, strides, W_init=W_init, b_init=None, name='depthwise')
            n = BatchNormLayer(n, decay=decay, act=tf.nn.relu6, is_train=train_bn, name='batchnorm1')
            n = Conv2d(n, n_filter, (1, 1), (1, 1), W_init=W_init, b_init=None, name='conv')
            n = BatchNormLayer(n, decay=decay, act=tf.nn.relu6, is_train=train_bn, name='batchnorm2')
        return n
    ...
    def stage(cnn, b1, b2, n_pos, maskInput1, maskInput2, is_train, name='stageX'):
        ...
        with tf.variable_scope(name):
            ...
            with tf.variable_scope("branch1"):
                b1 = depthwise_conv_block(net, 128, filter_size=(7, 7), name="c1")
                b1 = depthwise_conv_block(b1, 128, filter_size=(7, 7), name="c2")
                b1 = depthwise_conv_block(b1, 128, filter_size=(7, 7), name="c3")
                b1 = depthwise_conv_block(b1, 128, filter_size=(7, 7), name="c4")
                b1 = depthwise_conv_block(b1, 128, filter_size=(7, 7), name="c5")
                b1 = depthwise_conv_block(b1, 128, filter_size=(1, 1), name="c6")
                b1 = Conv2d(b1, n_pos, (1, 1), (1, 1), None, 'VALID', W_init=W_init, b_init=b_init2, name='conf')
                if is_train:
                    b1.outputs = b1.outputs * maskInput1
            with tf.variable_scope("branch2"):
                b2 = depthwise_conv_block(net, 128, filter_size=(7, 7), name="c1")
                b2 = depthwise_conv_block(b2, 128, filter_size=(7, 7), name="c2")
                b2 = depthwise_conv_block(b2, 128, filter_size=(7, 7), name="c3")
                b2 = depthwise_conv_block(b2, 128, filter_size=(7, 7), name="c4")
                b2 = depthwise_conv_block(b2, 128, filter_size=(7, 7), name="c5")
                b2 = depthwise_conv_block(b2, 128, filter_size=(1, 1), name="c6")
    ...
    with tf.variable_scope('model', reuse):
        ...
        n = BatchNormLayer(n, decay=decay, is_train=train_bn, act=tf.nn.relu, name='bn1')
